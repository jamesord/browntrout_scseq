...
#SBATCH --job-name="hisat2_trout_SE_311022.sh"
...
-p 8 --known-splicesite-infile $ref_path/Salmo_trutta.fSalTru1.1.104.filtered.splices.txt -U ../trimming/${sample}.trimmed.filtered.fastq.gz \
| samtools sort -@ 7 -o ${sample}_sorted.se.bam
sambamba view -F "not unmapped" -t 15 -f bam -o ${sample}_aligned_sorted.se.bam ${sample}_sorted.se.bam

rm ${sample}_sorted.se.bam
