...
#SBATCH --job-name="trinity_GG_trout_PE_311022.sh"
...
# make list of BAMs containing paired read alignments and merge those BAMs
ls $bam_path/*.pe.bam > bam.list.pe
samtools merge -@ 15 -o merged.pe.bam -b bam.list.pe

# get only properly paired read alignments
sambamba view -F "proper_pair" -t 15 -f bam -o merged.pe.paired.bam merged.pe.bam #get only the properly paired alignments
rm merged.pe.bam bam.list.pe

# run trinity with full cleanup enabled
Trinity --genome_guided_bam merged.pe.paired.bam \
--genome_guided_max_intron 10000 --CPU 16 --max_memory 60G --output $out_path/Trinity_trout_PE_311022 --full_cleanup --bflyCalculateCPU

# remove merged bam
rm merged.pe.paired.bam*

# recover output files from the workspace
mv $out_path/Trinity_trout_PE* ./