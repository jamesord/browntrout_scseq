...
p1<-FeaturePlot(immune.combined.sct.downsampled,features="ENSSTUG00000025205",min.cutoff = "q10", max.cutoff = "q90",raster=T)+
...
p2<-FeaturePlot(immune.combined.sct.downsampled,features="ENSSTUG00000006024",min.cutoff = "q10", max.cutoff = "q90",raster=T)+
...
p3<-FeaturePlot(immune.combined.sct.downsampled,features="ENSSTUG00000013074",min.cutoff = "q10", max.cutoff = "q90",raster=T)+
  theme(plot.title = element_text(hjust = 0))+theme(plot.title = element_text(face = "italic"))+
  theme(axis.line.x = element_blank())+theme(axis.line.y = element_blank())+
...
p4<-FeaturePlot(immune.combined.sct.downsampled,features="ENSSTUG00000019183",min.cutoff = "q10", max.cutoff = "q90",raster=T)+
  theme(plot.title = element_text(hjust = 0))+theme(plot.title = element_text(face = "italic"))+
  theme(axis.line.x = element_blank())+theme(axis.line.y = element_blank())+
...
pdf("FigureS_multicluster_markers_170724.pdf", width = 15, height = 4)
